...
% \node[anchor=south] at (1.2, 0.5) {Apple};
% \node[anchor=east] at (0.5, -0.2) {Banana};
% \node[anchor=north] at (-0.2, 0.5) {Carrot};
% \node[anchor=west] at (0.5, 1.2) {Durian};
\end{tikzpicture}
...
\vbox{\begin{lstlisting}
{
  ...
  addedDebts: {
    [asset]: amount, // for instance 'USD': 10,
  },
}
\end{lstlisting}}
...
\vbox{\begin{lstlisting}
{
  ...
  transactionId: <same as in initiate-update message it confirms>,
}
\end{lstlisting}}
...
\vbox{\begin{lstlisting}
{
  ...
  transaction: {
    ...
    asset: <something which the sender has to offer and which the receiver wants to receive>,
    amount: <1.0 for the entire asset described, between 0 and 1 for a fraction, >1 for multiple units>,
  },
  challenge: {
    name: 'ECDSA',
    namedCurve: 'P-256',
